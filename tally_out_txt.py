...
off_threshold = 0.10  # lowest state below this will turn them all off
...
def update_plot(frame):
    ...
    while True:
        ...
        if themax > name1threshold:
            ...
            cam_b = litoff(pintwo)
            print(f'{name1} on')
            msg_string = f"{name1} on"
            
        elif themax > name2threshold:
            cam_b = liton(pintwo)
            cam_a = litoff(pinone)
            print(f'{name2} on')
            msg_string = f"{name2} on"
...
if __name__ == '__main__':
    try:
        ...
        ax.text(0.05, 0.95, msg_string, transform=ax.transAxes, fontsize=14, verticalalignment='top', bbox=props)
